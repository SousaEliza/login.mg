import { AuthenticationService, UIService } from '../business/services/AuthenticationService.js';
import { EmailValidator, PasswordValidator } from '../business/validators/FormValidators.js';
...
export class LoginController {
    ...
    bindEvents() {
        // Login events
        this.inputMail.addEventListener('keyup', () => this.validateEmailField('email', 'errorMsg'));
        this.inputPwd.addEventListener('keyup', () => this.validatePasswordField('pwd', 'errorMsg'));
        this.loginBtn.addEventListener('click', (e) => this.handleLogin(e));

        // Registration events
        this.newEmail.addEventListener('keyup', () => this.validateEmailField('signin-email', 'errorMsgII'));
        this.newPwd.addEventListener('keyup', () => this.handlePasswordInput());
        this.confirmPwd.addEventListener('keyup', () => this.handlePasswordConfirmation());
        this.signUpBtn.addEventListener('click', (e) => this.handleRegistration(e));
        this.signInLink.addEventListener('click', () => this.uiService.showRegistrationView());
    ...
    async handleRegistration(event) {
        ...
        const result = await this.authService.register(name, email, password, confirmPassword);
